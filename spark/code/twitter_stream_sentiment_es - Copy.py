...
def get_prediction_json(key,rdd):
    ...
    tweet=rdd.map(lambda (key, value): json.loads(value)).map(
        lambda json_object: (json_object["text"].encode('utf-8'), str(json_object["created_at"]))
        )
...
#.writeStream.outputMode('append').format('org.elasticsearch.spark.sql').option('es.nodes', '10.0.100.51').option('es.port', 9200).start('tweets')
